...
The 4-inch target uses the Freenove-compatible RGB colour order and 80 MHz write / 20 MHz read SPI settings. The 2.8-inch target uses the ILI9341 profile and its native BGR order. LovyanGFX applies the panel rotation to touch coordinates at runtime, with a separate 180-degree touch-controller compensation for the FNK0104B mounting.
